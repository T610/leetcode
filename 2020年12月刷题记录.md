...
[希望用一种规律搞定背包问题](https://leetcode-cn.com/problems/target-sum/solution/xi-wang-yong-yi-chong-gui-lu-gao-ding-bei-bao-we-2/)

#### [240. 搜索二维矩阵 II](https://leetcode-cn.com/problems/search-a-2d-matrix-ii/)

gen剑指offer里面的题一样，要么从右上或者左下搜索

#### [152. 乘积最大子数组](https://leetcode-cn.com/problems/maximum-product-subarray/)

同时在维护一个最大和最小的dp数组，因为乘积里面有负负得正这种反转，并不单调，因此应该考虑负数的情况。
...
方法一的递归写法：要注意是左子树**所有**节点小于根节点，并不是左节点小于根节点就可以了

方法二：二叉搜索树的中序便利应该是递增的。

## 12.27

#### [1114. 按序打印](https://leetcode-cn.com/problems/print-in-order/)

多线程的题    newCondition()     +    .await()    +   .signal()    组合
...
synchronized   +  .wait()      +   .notifyAll()     进行等待
...
public class Foo {     
    ...
    public void first(Runnable printFirst) throws InterruptedException {
        synchronized (lock){
            //如果flag不为0则让first线程等待，while循环控制first线程如果满足条件就一直在while代码块中，防止出现中途跳入，执行下面的代码，其余线程while循环同理  
            while( flag != 0){  
                lock.wait();
            }
            // printFirst.run() outputs "first". Do not change or remove this line.
...
class Solution {
    public String longestPalindrome(String s) {
        ...
        String ans = "";
        for (int l = 0; l < n; ++l) {    //   最外层嵌套的是回文子串的长度
            for (int i = 0; i + l < n; ++i) {        //    里面的是回文子串的开始索引
                int j = i + l;
                if (l == 0) {
                    dp[i][j] = true;
                } else if (l == 1) {
